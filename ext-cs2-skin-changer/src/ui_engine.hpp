...
namespace SC_GUI {
    ...
    inline Theme currentTheme = {
        Color(255, 10, 10, 14),      // mainBg (Darker Deep Blue/Black)
        Color(255, 15, 15, 20),      // sidebarBg
        Color(255, 25, 25, 32),      // contentBg
        Color(255, 40, 40, 50),      // border
        Color(255, 35, 35, 45),      // separator
        Color(255, 130, 80, 255),    // accent (Modern Purple)
        Color(255, 240, 240, 255),   // text
        Color(255, 140, 140, 160)    // textDim
    };
    ...
    inline void DrawGlow(float x, float y, float w, float h, float r, Color color, int intensity = 15) {
        for (int i = 0; i < intensity; i++) {
            float alpha = (float)(intensity - i) / (float)intensity * (float)color.GetA() * 0.15f;
            Color c( (BYTE)alpha, color.GetR(), color.GetG(), color.GetB() );
            DrawStrokeRoundedRect(x - i, y - i, w + i * 2, h + i * 2, r + i, c, 1.5f);
        }
    }
    ...
    inline bool SkinCard(const std::string& id, const std::string& name, const std::string& image_url, int rarity, float x, float y, float w, float h, bool selected, bool diskEnabled = true) {
        ...
        if (selected) {
             ...
             DrawGlow(x, y, w, h, radius, currentTheme.accent, 8);
        }
        ...
        DrawFilledRoundedRect(x + 10, y + h - 6, w - 20, 3, 1.5f, rarityCol);
