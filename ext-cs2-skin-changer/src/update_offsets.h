...
namespace Updater {
    ...
    void UpdateOffsets() {
        std::cout << "[Updater] Checking for offset updates from sezzyaep/CS2-OFFSETS..." << std::endl;
        
        // 1. Client DLL Offsets (schemas)
        std::string clientData = FetchURL("https://raw.githubusercontent.com/sezzyaep/CS2-OFFSETS/main/client_dll.json");
        if (!clientData.empty()) {
            try {
        ...
        // 2. Global Offsets
        std::string offsetsData = FetchURL("https://raw.githubusercontent.com/sezzyaep/CS2-OFFSETS/main/offsets.json");
        if(!offsetsData.empty()) {
             try {
                ...
                if (client.find("dwViewAngles") != client.end()) { /* maybe add if needed */ }
                if (client.find("dwViewMatrix") != client.end()) { /* maybe add if needed */ }
